    ...
    if (!embedding_file_name.empty()) {
      ...
      if (fd < 0) {
        ...
        if (::fstat(fd, &st) == 0) {
          ...
          embedding_mmap_ptr =
            ::mmap(nullptr, embedding_mmap_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
void causallm::Gauss3_8_QNN::initialize_kv_cache() { kv_cache_.reset(); }
...
void causallm::Gauss3_8_QNN::saveKvCache(const std::string &cache_path) const {
...
void causallm::Gauss3_8_QNN::run(const WSTR prompt, bool do_sample,
  ...
  auto append_generation_token_to_kv_cache = [&](int token_to_append) {
    ...
    fill_generation_inputs_wrapper(token_to_append, current_kv_len);
  ...
  for (unsigned int c = 0; c < n_chunks; c++) {
    ...
    const int pos_ids_offset = current_kv_len * pos_dim;
  ...
  for (; idx < generation_full_kv_past_length; idx++) {
    ...
    token =
      sample(std::get<uint16_t *>(outputs[generation_logits_output_index]),
             vocab_size, input.data(), input.size(), logit_scale, logit_offset,
  ...
  if (log_output) {
    const int generated = std::max(1, static_cast<int>(generated_tokens));
    std::cout << std::endl;
    std::cout << std::endl;
    std::cout << "Generation exec_time : " << this->raw_exec_seconds.count()
              << ", token per second: "
              << generated / this->raw_exec_seconds.count()
