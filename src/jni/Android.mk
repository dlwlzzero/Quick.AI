...
# QNN infra sources live directly under models/qnn/. Per-model sources live in
# model subdirectories (models/<m>/ and models/qnn/<m>/) and are globbed
# generically — the public tree ships only gemma4-e2b-qnn, and any proprietary
# model directory dropped into the tree (and git-ignored) is picked up with no
# edits here. Globs expand to empty when no such directories are present.
QUICK_DOT_AI_QNN_INFRA_SRC := $(wildcard $(QUICK_DOT_AI_MODELS_ROOT)/qnn/*.cpp)
QUICK_DOT_AI_EXT_SRC := \
    $(filter-out $(QUICK_DOT_AI_QNN_INFRA_SRC),$(wildcard $(QUICK_DOT_AI_MODELS_ROOT)/*/*.cpp)) \
    $(wildcard $(QUICK_DOT_AI_MODELS_ROOT)/qnn/*/*.cpp)
QUICK_DOT_AI_EXT_INCS := \
    $(wildcard $(QUICK_DOT_AI_MODELS_ROOT)/*/) \
    $(wildcard $(QUICK_DOT_AI_MODELS_ROOT)/qnn/*/)
...
# main.cpp + all CausalLM sources + model_callbacks (registry for model plugin TU constructors)
LOCAL_SRC_FILES := \
