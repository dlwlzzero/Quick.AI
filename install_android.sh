...
adb push "$NNTRAINER_ANDROID/libnntrainer.so" $INSTALL_DIR/
adb push "$NNTRAINER_ANDROID/libccapi-nntrainer.so" $INSTALL_DIR/
...
for f in libcausallm.so libquick_dot_ai.so; do
    [ -f "$BUILD_DIR/src/$f" ] && adb push "$BUILD_DIR/src/$f" $INSTALL_DIR/
done

if [ -f "$BUILD_DIR/src/quick_dot_ai" ]; then
    adb push "$BUILD_DIR/src/quick_dot_ai" $INSTALL_DIR/
    adb shell "chmod 755 $INSTALL_DIR/quick_dot_ai"
fi

# api target
[ -f "$BUILD_DIR/api/libquick_dot_ai_api.so" ] && \
    adb push "$BUILD_DIR/api/libquick_dot_ai_api.so" $INSTALL_DIR/

# api-test target
if [ -f "$BUILD_DIR/api-app/quick_dot_ai_test" ]; then
    adb push "$BUILD_DIR/api-app/quick_dot_ai_test" $INSTALL_DIR/
    adb shell "chmod 755 $INSTALL_DIR/quick_dot_ai_test"
fi

# qnn target
[ -f "$BUILD_DIR/qnn/libqnn_context.so" ] && \
    adb push "$BUILD_DIR/qnn/libqnn_context.so" $INSTALL_DIR/
