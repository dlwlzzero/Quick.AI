...
/**
 * Per-architecture callbacks registered by proprietary model plugin TU files.
 * When a plugin TU is absent (public build), no callbacks are registered for
 * that architecture; callers should fall back to CAUSAL_LM_ERROR_UNSUPPORTED.
 */
struct ModelCallbacks {
   ...
   * The plugin TU casts it to CausalLmModel* and accesses h.models[0]/[1].
   */
...
/**
 * Registry keyed by architecture name string (the model's arch_string).
 * Proprietary model plugin TUs call register_for() at static-init time.
...
public:
   ...
   * Returns nullptr if not registered (no plugin TU for this architecture).
   */
