...
static const char *get_model_name_from_type(ModelType type) {
  if (type == CAUSAL_LM_MODEL_QWEN3_0_6B)
    return "QWEN3-0.6B";
...
ErrorCode setOptions(Config config) {
  ...
  quick_dot_ai::g_use_chat_template = config.use_chat_template;
  quick_dot_ai::g_verbose = config.verbose;
  if (config.debug_mode) {
    // Ensure models are registered so we can validate them
    quick_dot_ai::register_models();
    quick_dot_ai::validate_models();
  }
...
ErrorCode registerModelArchitecture(const char *arch_name,
  ...
  quick_dot_ai::register_arch(arch_name, config);
  return CAUSAL_LM_ERROR_NONE;
}
...
ErrorCode registerModel(const char *model_name, const char *arch_name,
  ...
  if (model_name == nullptr || arch_name == nullptr)
    return CAUSAL_LM_ERROR_INVALID_PARAMETER;
  std::lock_guard<std::mutex> lock(quick_dot_ai::g_mutex);
  quick_dot_ai::register_model(model_name, arch_name, config);
  return CAUSAL_LM_ERROR_NONE;
}
...
ErrorCode loadModel(BackendType compute, ModelType modeltype,
  ...
  try {
    ...
    quick_dot_ai::json cfg;
    quick_dot_ai::json generation_cfg;
    quick_dot_ai::json nntr_cfg;
    std::string model_dir_path;

    // Check in-memory map first
    if (quick_dot_ai::g_model_registry.find(lookup_name) !=
        quick_dot_ai::g_model_registry.end()) {
      // ------------------------------------------------------------------------
    ...
    quick_dot_ai::g_model->initialize();
    quick_dot_ai::g_model->load_weight(weight_file);

    quick_dot_ai::g_initialized = true;
    quick_dot_ai::g_architecture = architecture;
...
ErrorCode runModel(const char *inputTextPrompt, const char **outputText) {
  if (!quick_dot_ai::g_initialized || !quick_dot_ai::g_model) {
...
// We assume single batch request for this API
#if defined(_WIN32)
    quick_dot_ai::g_model->run(std::wstring(input.begin(), input.end()), false,
                               L"", L"", quick_dot_ai::g_verbose);
#else
    quick_dot_ai::g_model->run(input, false, "", "", quick_dot_ai::g_verbose);
#endif
    ...
    *outputText = quick_dot_ai::g_last_output.c_str();
