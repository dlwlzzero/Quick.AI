...
LOCAL_SRC_FILES := \
    ...
    ../model_descriptors_public.cpp

LOCAL_SHARED_LIBRARIES := nntrainer ccapi-nntrainer causallm quick_dot_ai
LOCAL_LDLIBS := -llog -landroid -ldl

# Include every model subdirectory present so the API TUs can resolve plugin
# headers. Globbed generically (public tree ships only gemma4-e2b-qnn; any
# proprietary model directory dropped in is picked up automatically).
LOCAL_C_INCLUDES += $(CAUSALLM_INCLUDES) \
    $(LOCAL_PATH)/.. \
    $(QUICK_DOT_AI_ROOT)/models/qnn \
    $(wildcard $(QUICK_DOT_AI_ROOT)/models/*/) \
    $(wildcard $(QUICK_DOT_AI_ROOT)/models/qnn/*/)
