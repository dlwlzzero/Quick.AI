...
Only these six fields are serialized. The descriptor's `config_name` and
`arch_string` fields are internal (used to resolve the config and the nntrainer
Factory architecture) and are **not** included in the catalog JSON.
...
typedef struct {
  ...
  const char *config_name;   // internal: config-registry key (not in catalog JSON)
  const char *arch_string;   // internal: nntrainer Factory key (not in catalog JSON)
} ModelDescriptor;
```

The struct also includes `config_name` and `arch_string`, which the catalog
JSON does not expose.
...
int main() {
  ...
  ErrorCode err = loadModelHandleByName(CAUSAL_LM_BACKEND_CPU,
                                        "qwen3-0.6b",
...
These functions accept a preprocessed CHW float pixel buffer (`pixelValues`,
shape `3 × H × W`) plus patch metadata. 
...
For models that accept several images in one prompt (e.g. the to be supported `vjepa-qnn`), use the
