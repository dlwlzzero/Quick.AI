...
if [ "$PLATFORM" = "android" ]; then
    ...
    if [ "$CLEAN" = true ] || [ ! -f "$NNTRAINER_ANDROID_LIB" ]; then
        ...
        #
        # -Dprefix: package_android.sh runs `ninja install`. nntrainer's own
        # Android artifacts install to an absolute path (builddir/android_build_result,
        # see nntrainer/meson.build), independent of the meson prefix. However the
        # googletest subproject (pulled in unconditionally when no system gtest/gmock
        # is found) still honors the meson prefix and tries to install libgmock.a /
        # libgtest.a to the default /usr/local/lib, which fails for a non-root user
        # with "PermissionError: [Errno 13] Permission denied". Point the prefix at a
        # writable, in-tree staging dir so the install succeeds without root.
        NNTRAINER_STAGE_PREFIX="$SCRIPT_DIR/.nntrainer_android_stage"
        NNTRAINER_EXTRA_OPTS="-Dmmap-read=false -Dprefix=$NNTRAINER_STAGE_PREFIX"
